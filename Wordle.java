public class Wordle {
    ...
    public static String[] readDictionary(String filename) {
        ...
        In inRead = new In(filename);
        String[] arrDictionary = new String[n];
        for(int i = 0; i < n; i++) {
            arrDictionary[i] = inRead.readString();
        }
    ...
    public static void computeFeedback(String secret, String guess, char[] resultRow) {
        for(int i = 0; i < secret.length(); i++) {
            ...
            if      (secret.charAt(i) == temp)      resultRow[i] = 'G';
            else  if(containsChar(secret, temp))    resultRow[i] = 'Y';
            else                                    resultRow[i] = '_';
        }
    }
